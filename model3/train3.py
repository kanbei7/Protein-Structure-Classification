...
Random Guess baseline is at most 12%
'''
...
from model3 import BiDirTBLSTM
...
def load_label2idx():
	global label2idx
	global n_classes
	df = pd.read_csv(path + 'selected79_fold_sp.csv')
...
def test(epoch_idx):
	...
	with torch.no_grad():
		for b_idx, X, y in test_loader:
			...
			n_total += y.size(0)
	#auc_pr = average_precision_score(y_test, y_score)
	print("[Test: {}] Loss:{:.4f}, Acc:{:.4f}".format(epoch_idx, total_loss, n_correct / n_total))
...
if __name__ == '__main__':
	...
	model = BiDirTBLSTM(vocab_size, embedding_dim, n_classes,rnn_hidden_size, batch_size, pbatch_size=pbatch_size)
